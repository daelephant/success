...
define(["jquery", "plugins", "grid", "dialog", "datepicker"], function(require, exports){
	...
	*/ 
					...
					if(isSkuSingle){
						...
						for(var i = 0 ,len = goodsPropList.length; i<len; i++){
							...
							if(su){
								...
								!$thisRow.data('storageInfo') && $thisRow.data('storageInfo', { 
									id: tempRowData.locationId, 
									name: tempRowData.locationName
								});
								!$thisRow.data('unitInfo') && $thisRow.data('unitInfo',{
									unitId: tempRowData.unitId,
									name: tempRowData.unitName
								});
							}
	...
	Business.moreFilterEvent = function(){
			$(window).bind('resize',function(){
				var winWidth = $(window).width() - 38;
				var Width = $('.mod-search .l').width();
		    if(Width >= winWidth){
		    	if($('#conditions-trigger').hasClass('conditions-expand')){
		    		$('#conditions-trigger').addClass("ui-btn").html('更多<b></b>');
		    	}
		    	else{
		    		$('#conditions-trigger').addClass("ui-btn").html('收起<b></b>');
		    	}
		    }
		    else{
		    	$('#conditions-trigger').removeClass("ui-btn").html('');
		    }
		});
		$('#conditions-trigger').on('click', function(e){
			e.preventDefault();
		  if (!$(this).hasClass('conditions-expand')) {
				$('.mod-search .l ul').attr('style','overflow:hidden;height:35px;padding:5px 250px 5px 0;');
				var winWidth = $(window).width()-38;
				var Width = $('.mod-search .l').width();
				if(Width >= winWidth || Width >= 1650){
					$('#conditions-trigger').addClass('conditions-expand ui-btn').html('更多<b></b>');
				}
				else{
					$('#conditions-trigger').removeClass("ui-btn").addClass('conditions-expand').html('');
				}			
		  } else {
				$('.mod-search .l ul').attr('style','overflow:visible;height:auto;padding:5px 250px 5px 0;');
				$('#conditions-trigger').removeClass('conditions-expand').addClass('ui-btn').html('收起<b></b>');
		  };
		});
	};
	...
	var html = [
              ...
              '</li>',
							...
							'<li id="billNum">',
								...
								'<input type="text" value="请输入单号查询" class="ui-input ui-input-ph" name="matchCon" id="matchCon" />',
							'</li>',
							'<li id="filter">',
              ...
              '<li class="chk-list" id="profit-wrap">',
                '<span class="chk"><input type="checkbox" value="warehouse" name="warehouse" id="warehouse" checked/>在库</span>',
              ...
              '</li>',
					...
					'<a href="#" id="conditions-trigger" class="ui-btn mrb conditions-trigger" tabindex="-1">更多<b></b></a>',
					'<a class="ui-btn mrb" id="filter-submit">查询</a>',
					'<a href="#" class="ui-btn ui-btn-sp mrb fl" id="btn-print">打印</a>',
					'<a href="#" class="ui-btn ui-btn-sp fl" id="btn-export">导出</a></div>'].join('');
