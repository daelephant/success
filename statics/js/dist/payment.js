...
var THISPAGE = {
	...
	initDom: function(a) {
		...
		var b = '<a id="savaAndAdd" class="ui-btn ui-btn-sp mrb">保存并新增</a><a id="save" class="ui-btn">保存</a>',
			c = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/payment.do?action=toPdf" target="_blank" id="print" class="ui-btn mrb">打印</a><a id="edit" class="ui-btn mrb">保存</a>',
			d = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/payment.do?action=toPdf" target="_blank" id="print" class="ui-btn mrb">打印</a><b></b></a>',
			e = "",
			f = "",
			g = "";
	...
	addEvent: function() {
		...
		this.customerCombo.input.enterKey(), this.$_note.enterKey(), this.$_date.bind("keydown", function(a) {
			...
			if (Business.verifyRight("PAYMENT_ADD")) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/payment.do?action=add", {
			...
			if (b.preventDefault(), Business.verifyRight("PAYMENT_UPDATE")) {
				...
				c && Public.ajaxPost("../scm/payment.do?action=updatePayment", {
			...
			if (Business.verifyRight("PAYMENT_ADD")) {
				var d = THISPAGE.getPostData();
				d && (c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/payment.do?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
			...
			$.dialog({
				...
				height: 510,
				title: "选择源单",
				content: "url:../scm/payment.do?action=initUnhxList",
				data: {
					url: "../scm/invPu.do?action=findUnhxList&buId=" + originalData.buId + "&id=" + originalData.id
				},
				lock: !0,
				ok: function() {
			...
			if (b.preventDefault(), Business.verifyRight("PAYMENT_CHECK")) {
				...
				d && c.ajaxPost("../scm/payment.do?action=checkPayment", {
					postData: JSON.stringify(d)
				}, function(b) {
			...
			if (b.preventDefault(), Business.verifyRight("PAYMENT_UNCHECK")) {
				...
				c.ajaxPost("../scm/payment.do?action=rsbatchCheckPayment", {
					id: originalData.id
				}, function(b) {
	...
	originalData;
$(function() {
	urlParam.id ? hasLoaded || Public.ajaxGet("../scm/payment.do?action=update", {
		id: urlParam.id
	}, function(a) {
