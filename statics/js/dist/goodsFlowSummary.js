define(["jquery", "print"], function(a) {
	function b() {
		Business.getSearchList(), Business.filterGoods(), Business.filterStorage(), Business.moreFilterEvent(), h("#date,#storage,#goodsfilter,#goods").show(), h("#storage").insertBefore(h("#goods")), h("#btn-print").hide(), h("#conditions-trigger").trigger("click"), chkboxes = h("#chk-wrap").cssCheckbox(), j.beginDate && j.endDate && h(".grid-subtitle").text(j.beginDate + "至" + j.endDate), i.enableAssistingProp || h("#chk-wrap").hide(), "1" === j.showSku && h('#chk-wrap input[name="showSku"]').attr("checked", !0), h("#filter-fromDate").val(j.beginDate), h("#filter-toDate").val(j.endDate), h("#filter-goods input").val(j.goodsNo), h("#filter-storage input").val(j.storageNo), Public.dateCheck(), this.fDatePicker = new Pikaday({
	...
	function c() {
		var a = this;
		h("#filter-submit").click(function(b) {
			b.preventDefault();
			...
			for (var i = 0, l = chkVals.length; l > i; i++) j[chkVals[i]] = 1;
			var m = h.dialog.tips("正在查询，请稍候...", 1e3, "loading.gif", !0);
			Public.ajaxPost("../report/deliverSummary.do?action=detail", j, function(a) {
				200 === a.status ? (h(".no-query").remove(), h(".ui-print").show(), h(".grid-subtitle").text(j.beginDate + "至" + j.endDate), d(a.data), m.close()) : (m.close(), parent.Public.tips({
					type: 1,
					content: msg
				}))
			})
		}), j.search && h("#filter-submit").trigger("click"), h("#btn-print").click(function(a) {
			...
			if (a.preventDefault(), Business.verifyRight("DeliverSummaryReport_EXPORT")) {
				...
				Business.getFile("../report/deliverSummary.do?action=exporter", b)
			}
		}), h("#config").show().click(function() {
			m.config()
		})
	}
	function d(a) {
		...
		for (var b = "auto", c = [{
			...
			frozen: !0,
			width: 100
		}], d = a.colIndex, e = a.colNames, i = a.stoNames, k = [], n = "", o = 0, p = 6, q = d.length; q > p; p++) {
			var r = null;
			r = {
				name: d[p],
				label: e[p],
				width: 80,
				align: "right"
			}, c.push(r), d[p].split("_")[1] === n ? (k.pop(), k.push({
				startColumnName: d[p - 1],
				numberOfColumns: 2,
				titleText: i[o - 1]
			})) : (k.push({
				startColumnName: d[p],
		...
		m.gridReg("grid", c), c = m.conf.grids.grid.colModel, h("#grid").jqGrid({
			ajaxGridOptions: {
				complete: function() {}
			},
			data: a.rows,
	...
	var h = a("jquery"),
		...
		l = 0;
	a("print");
	var m = Public.mod_PageConfig.init("goodsFlowSummary");
	b(), c();
	var n;
