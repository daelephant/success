var curRow, curCol, curArrears, loading, urlParam = Public.urlParam(),
	...
	THISPAGE = {
		init: function(a) {
			this.mod_PageConfig = Public.mod_PageConfig.init("transfers"), this.loadGrid(a), this.initDom(a), this.initCombo(), this.addEvent(), a.id > 0 && a.checked ? this.disableEdit() : (this.editable = !0, $("#grid").jqGrid("setGridParam", {
				cellEdit: !0
			})), $.cookie("BarCodeInsert") && THISPAGE.$_barCodeInsert.addClass("active"), this.goodsEdittypeInit()
		},
		initDom: function(a) {
			...
			var b = '<a id="savaAndAdd" class="ui-btn ui-btn-sp mrb">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				c = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/invTf.do?action=toPdf" target="_blank" id="print" class="ui-btn mrb">打印</a><a id="edit" class="ui-btn mrb">保存</a>',
				d = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/invTf.do?action=toPdf" target="_blank" id="print" class="ui-btn mrb">打印</a><b></b></a>',
				e = "",
				f = "",
				g = "";
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/invTf.do?action=add", {
				...
				if (b.preventDefault(), Business.verifyRight("TF_UPDATE")) {
					...
					c && Public.ajaxPost("../scm/invTf.do?action=updateInvTf", {
				...
				var d = THISPAGE.getPostData();
				d && (c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/invTf.do?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("TF_CHECK")) {
					...
					d && c.ajaxPost("../scm/invTf.do?action=checkInvTf", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("TF_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invTf.do?action=rsBatchCheckInvTf", {
						id: originalData.id
					}, function(b) {
	...
	originalData;
$(function() {
	urlParam.id ? hasLoaded || Public.ajaxGet("../scm/invTf.do?action=update", {
		id: urlParam.id
	}, function(a) {
