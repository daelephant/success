define(["jquery", "print"], function(a, b, c) {
	function d() {
		m.matchCon ? k("#matchCon").val(m.matchCon || "请输入接收单位、供应商或编号查询") : (k("#matchCon").addClass("ui-input-ph"), k("#matchCon").placeholder()), m.customer && k("#customer").attr("checked", !0), m.supplier && k("#supplier").attr("checked", !0), k("#search").on("click", function(a) {
			a.preventDefault();
			var b = "请输入接收单位、供应商或编号查询" === k("#matchCon").val() ? "" : k.trim(k("#matchCon").val());
			m = {
				matchCon: b,
	...
	var k = a("jquery"),
		...
		n = "../report/contactDebt.do?action=exporter",
		o = "../report/contactDebt.do?action=detail";
	a("print");
