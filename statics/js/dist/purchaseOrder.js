var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		init: function(a) {
			this.mod_PageConfig = Public.mod_PageConfig.init("purchaseOrder"), isAdmin !== !1 || SYSTEM.rights.AMOUNT_INAMOUNT || (hiddenAmount = !0, $("#amountArea").hide()), this.loadGrid(a), this.initDom(a), this.initCombo(), a.id > 0 && a.checked ? this.disableEdit() : (this.editable = !0, $("#grid").jqGrid("setGridParam", {
				cellEdit: !0
		...
		initDom: function(a) {
			...
			var c = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				...
				f = "",
				g = "",
				h = "150502" == originalData.transType ? "生成退货单" : "生成入库单",
			...
			if (this.btn_add = c, this.btn_edit = d, this.btn_audit = f, this.btn_view = e, this.btn_reaudit = g, this.btn_turn = i, this.btn_close = j, this.btn_open = k, a.id > 0) {
				...
				}), "list" !== urlParam.flag && (l = ""), a.isDelete && $("#mark").addClass("has-closed"), "edit" === a.status) {
					var m = d + f;
		...
		addEvent: function() {
			...
			this.customerCombo.input.enterKey(), this.$_date.bind("keydown", function(b) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost("../scm/invPo.do?action=add", {
				...
				var c = $(this),
					d = THISPAGE.getPostData();
				d && c.ajaxPost("../scm/invPo.do?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
					if (200 === b.status) {
				...
				if (b.preventDefault(), Business.verifyRight("PO_ADD")) {
					var c = $(this);
				...
				if (b.preventDefault(), Business.verifyRight("PO_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invPo.do?action=updateInvPo", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PO_CHECK")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invPo.do?action=checkInvPo", {
						postData: JSON.stringify(d)
					}, function(b) {
						200 === b.status ? (originalData.id = b.data.id, $("#mark").addClass("has-audit"), a.$_checkName.html(SYSTEM.realName).parent().show(), a.disableEdit(), $("#groupBtn").html(a.btn_turn + a.btn_view + a.btn_reaudit + a.btn_close), "150502" == a.classes.getValue() ? $("#turn").html("生成退货单") : $("#turn").html("生成入库单"), parent.Public.tips({
							content: "审核成功！"
						})) : parent.Public.tips({
							type: 1,
							content: b.msg
				...
				if (b.preventDefault(), Business.verifyRight("PO_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invPo.do?action=rsBatchCheckInvPo", {
						id: originalData.id
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PO_CLOSE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invPo.do?action=batchClose", {
						id: originalData.id
					}, function(b) {
						200 === b.status && b.msg && b.msg.length ? 1 == b.msg[0].isSuccess ? ($("#mark").addClass("has-closed'has-closed'"), a.disableEdit(), $("#groupBtn").html(a.btn_view + a.btn_open), "150502" == a.classes.getValue() ? $("#turn").html("生成退货单") : $("#turn").html("生成入库单"), parent.Public.tips({
							content: "关闭成功！"
						})) : parent.Public.tips({
							type: 1,
				...
				if (b.preventDefault(), Business.verifyRight("PO_UNCLOSE")) {
					var c = $(this);
					c.ajaxPost("../scm/invPo.do?action=rebatchClose", {
						id: originalData.id
					}, function(b) {
						200 === b.status && b.msg && b.msg.length ? 1 == b.msg[0].isSuccess ? ($("#mark").removeClass("has-closed"), a.enableEdit(), $("#groupBtn").html((billRequiredCheck ? a.btn_turn : "") + a.btn_edit + a.btn_reaudit + a.btn_close), "150502" == a.classes.getValue() ? $("#turn").html("生成退货单") : $("#turn").html("生成入库单"), parent.Public.tips({
							content: "开启成功！"
						})) : parent.Public.tips({
							type: 1,
				...
				parent.tab.addTabItem({
					tabid: d,
					text: c,
					url: "../scm/invPu.do?action=initPur&id=" + originalData.id + "&flag=list&turn&transType=" + e
				}), parent.tab.reload(d)
			}), $("#prev").click(function(b) {
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invPo.do?action=update", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invPo.do?action=update", {
					id: a.idList[a.idPostion]
				}, function(b) {
...
$(function() {
	if (urlParam.id) {
		if (!hasLoaded) {
			...
			} else Public.ajaxGet("../scm/invPo.do?action=update", {
				id: urlParam.id
			}, function(b) {
