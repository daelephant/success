define(["jquery", "print"], function(a, b, c) {
	function d() {
		Business.getSearchList(), Business.filterSupplier(), Business.moreFilterEvent(), k("#date,#filter,#supplier,#chk-blank").show(), k("#filter label").text("供应商类别"), k("#chk-blank i").text("不显示无欠款供应商"), k("#conditions-trigger").trigger("click"), k("#filter-fromDate").val(m.beginDate || ""), k("#filter-toDate").val(m.endDate || ""), k("#filter-supplier input").val(m.customerNo || ""), m.beginDate && m.endDate && k("div.grid-subtitle").text("日期: " + m.beginDate + " 至 " + m.endDate), k("#filter-fromDate, #filter-toDate").datepicker(), Public.dateCheck(), chkblank = k("#chk-blank").cssCheckbox();
	...
	var k = a("jquery"),
		...
		n = "../report/fundBalance.do?action=exporterSupplier&type=10",
		o = "../report/fundBalance.do?action=detailSupplier&type=10";
	a("print");
