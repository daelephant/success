...
function addHoverDom(a, b) {
	...
	if (0 != b.id) {
		...
		return i && i.bind("click", function(a) {
			a.stopPropagation();
			var c = myTree.zTree;
			return Public.ajaxPost("../basedata/unitType.do?action=delete", {
				id: b.id
			}, function(a) {
				if (a && 200 == a.status) {
					parent.Public.tips({
	...
	handle = {
		...
		del: function(a) {
			...
			$.dialog.confirm("删除的计量单位将不能恢复，请确认是否删除？", function() {
				Public.ajaxPost("../basedata/unit.do?action=delete", {
					id: a
				}, function(b) {
					if (b && 200 == b.status) {
						parent.Public.tips({
...
var myTree = {
	...
	init: function(a, b, c, d) {
		if (0 !== a.length) {
			var e = this;
			e.opts = $.extend(!0, e.opts, b), e.container = $(a), e.obj = $(e._getTemplate(b)), e.container.append(e.obj), c = $.extend(!0, e.setting, c);
			for (var f = [], g = 0; g < defaultPage.SYSTEM.unitGroupInfo.length; g++) {
				var h = defaultPage.SYSTEM.unitGroupInfo[g];
				h.parentId = 0, f.push(h)
			}
			return e._callback(f), e
		}
	},
