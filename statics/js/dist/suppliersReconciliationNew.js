define(["jquery", "print"], function(a) {
	function b() {
		i("#match").cssCheckbox(), Business.moreFilterEvent(), i("#date,#saleCustomer,#match").show(), i("#saleCustomer label").text("入库单位"), i("#conditions-trigger").trigger("click"), "true" == k.showDetail && (i("#match").find("label").addClass("checked"), i("#match").find("input")[0].checked = !0), k.beginDate && k.endDate && i("div.grid-subtitle").text("日期: " + k.beginDate + "至" + k.endDate), i("#filter-fromDate").val(k.beginDate), i("#filter-toDate").val(k.endDate), i("#customerSale input").val(k.customerName), Business.supplierCombo(i("#customerSale"), {
			defaultSelected: 0,
			addOptions: {
				text: p,
				value: 0
		...
		i("#filter-submit").on("click", function(c) {
			...
			return g.getTime() > j.getTime() ? void parent.Public.tips({
				...
				var b = a.data,
					...
					k = b.county || "",
					l = b.deliveryAddress || "";
				c = "<i>联系人：" + c, d = "</i><i>电话：" + d + "/" + g, address = "</i></p><p>地址：" + h + j + k + l, date = "<i class='fr'>日期: " + e + " 至 " + f + "</i></p>", i("div.grid-subtitle").html("<p>供应商：" + n + c + d + address + date)
			}), void h(q))
		}), i("#filter-reset").on("click", function(a) {
			a.preventDefault(), i("#filter-fromDate").val(""), i("#filter-toDate").val(""), i("#customerSale input").val("")
		})
	}
	function c() {
		i("#btn-print").click(function(a) {
			...
			if (i(this).data("hasInstance")) this.customerDialog.show().zindex();
				...
				var a = i("#customerSale").prev().text(),
					b = "选择" + a;
				if ("供应商" === a || "入库单位" === a) var c = "url:../settings/select_customer?type=10&multiselect=false";
	...
	var l = "../report/supplierBalance.do?action=exporter",
		m = "../report/supplierBalance.do?action=detail",
		n = i("#customerSale"),
		o = (i("#match"), i("#match").find("input"), o || {});
	o.$_customer = n, this.THISPAGE = o;
	var p = "（请选择入库单位）";
	a("print");
	var q = Public.mod_PageConfig.init("suppliersReconciliation");
	b(), c(), d();
	var r;
