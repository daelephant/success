var curRow, curCol, loading, urlParam = Public.urlParam(),
	...
	THISPAGE = {
		init: function(a) {
			this.mod_PageConfig = Public.mod_PageConfig.init("otherWarehouse"), SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_INAMOUNT || (hiddenAmount = !0), this.loadGrid(a), this.initDom(a), this.initCombo(), this.addEvent(), $.cookie("BarCodeInsert") && THISPAGE.$_barCodeInsert.addClass("active"), this.goodsEdittypeInit()
		},
		initDom: function(a) {
			...
			var c = "",
				d = "";
			billRequiredCheck && (c = '<a class="ui-btn" id="audit">审核</a>', d = '<a class="ui-btn" id="reAudit">反审核</a>'), this.btn_audit = c, this.btn_reaudit = d, this.btn_add = '<a id="savaAndAdd" class="ui-btn ui-btn-sp mrb">保存并新增</a><a id="save" class="ui-btn">保存</a>', this.btn_edit = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a target="_blank" id="print" class="ui-btn mrb">打印</a><a id="edit" class="ui-btn">保存</a>', this.btn_view = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a target="_blank" id="print" class="ui-btn mrb">打印</a>', this.btn_p_n = '<a class="ui-btn-prev mrb" id="prev" title="上一张"><b></b></a><a class="ui-btn-next" id="next" title="下一张"><b></b></a>', a.id > 0 ? (this.$_customer.data("contactInfo", {
				id: a.buId,
				name: a.contactName
		...
		loadGrid: function(a) {
			function b(a, b, c) {
				return a ? (z(b.rowId), a) : c.invNumber ? c.invSpec ? c.invNumber + " " + c.invName + "_" + c.invSpec : c.invNumber + " " + c.invName : "&#160;"
			}
			function c(a, b) {
				var c = $(".goodsAuto")[0];
				return c
			...
			function f(a, b, c) {
				...
				if ((c.isSerNum && "0" != c.isSerNum || e.isSerNum && "0" != e.isSerNum || c.invSerNumList) && "合计" !== c.goods && (c.isSerNum || "1" == d.isSerNum || c.invSerNumList)) {
					var f = $("#" + b.rowId).data("goodsInfo");
					a = f && f.serNumList && f.serNumList.length > 0 || c.invSerNumList ? "<span class='qtyInsertAfter'>SN</span>" + a : "<span class='qtyInsert'>SN</span>" + a
				}
				return a || "&#160;"
			}
			function g(a) {
			...
			function z(a) {
				...
				if (b) {
					if (b.batch || $("#grid").jqGrid("setCell", a, "batch", "&#160;"), b.safeDays || ($("#grid").jqGrid("setCell", a, "prodDate", "&#160;"), $("#grid").jqGrid("setCell", a, "safeDays", "&#160;"), $("#grid").jqGrid("setCell", a, "validDate", "&#160;")), 1 == b.isWarranty && $("#grid").jqGrid("showCol", "batch"), b.safeDays > 0 && ($("#grid").jqGrid("showCol", "prodDate"), $("#grid").jqGrid("showCol", "safeDays"), $("#grid").jqGrid("showCol", "validDate")), b.prices && b.purUnitName) {
						for (var c = 0; c < b.prices.length; c++) b.prices[c].unitId == b.purUnitId && (b.price = b.prices[c].purPrice, b.amount = b.price * b.qty);
						b.mainUnit = b.purUnitName, b.unitId = b.purUnitId
					}
					var d = {
						skuName: b.skuName || "",
					...
					SYSTEM.ISSERNUM && b.isSerNum && (d.qty = d.serNumList ? d.serNumList.length : 0), d.amount = d.amount ? d.amount : d.price * d.qty;
					var e = (Number(d.amount), $("#grid").jqGrid("setRowData", a, d));
					e && THISPAGE.calTotal()
				}
			}
			var A = this,
				B = (new Date).format();
			if (a.id) {
				for (var C = 0; C < a.entries.length; C++) a.entries[C].id = C + 1;
				var D = 8 - a.entries.length;
				if (D > 0) for (var C = 0; D > C; C++) a.entries.push({})
			}
			A.newId = 9;
			var E = "grid",
				F = [{
					...
					unformat: function(a, b, c) {
						var d, e, f = (b.colModel.formatter, b.colModel.formatoptions || {}),
							g = /([\.\*\_\'\(\)\{\}\+\?\\])/g,
							f = $.extend({}, ($.jgrid.formatter || {}).currency, f);
						return e = f.thousandsSeparator.replace(g, "\\$1"), stripTag = new RegExp(e, "g"), d = $(c).text(), f.prefix && f.prefix.length && (d = d.substr(f.prefix.length)), f.suffix && f.suffix.length && (d = d.substr(0, d.length - f.suffix.length)), d = d.replace(stripTag, "").replace(f.decimalSeparator, ".").replace("SN", "")
					},
					formatter: f,
					formatoptions: {
			...
			A.mod_PageConfig.gridReg(E, F), F = A.mod_PageConfig.conf.grids[E].colModel, $("#grid").jqGrid({
				data: a.entries,
				datatype: "clientSide",
				autowidth: !0,
				height: "100%",
				...
				loadComplete: function(a) {
					if (THISPAGE.$_barCodeInsert = $("#barCodeInsert"), urlParam.id > 0 || urlParam.cacheId) {
						var b = a.rows,
							c = b.length;
						A.newId = c + 1;
						for (var d = 0; c > d; d++) {
							...
							Business.cacheManage.getGoodsInfoByNumber(g.number, function(a) {
								g.isSerNum = a.isSerNum, g.isWarranty = f.isWarranty = a.isWarranty, g.safeDays = f.safeDays = a.safeDays, g.invSkus = a.invSkus, g.prices = a.prices, g.id = f.invId, $("#" + e).data("goodsInfo", g).data("storageInfo", {
									id: f.locationId,
									name: f.locationName
								}).data("unitInfo", {
									unitId: f.unitId,
				...
				afterEditCell: function(a, b, c, d, e) {
					...
					if ("qty" === b) {
						...
						var g = $("#" + a).data("goodsInfo");
						if (!g) return;
						$("#" + d + "_qty", "#grid").val(c), SYSTEM.ISSERNUM && g.isSerNum && Business.serNumManage({
							row: $("#" + a),
							isCreate: !0
						})
					}
							...
							return THISPAGE.$_barCodeInsert && THISPAGE.$_barCodeInsert.hasClass("active") ? Business.cacheManage.getGoodsInfoByBarCode(c, h, !0) : Business.cacheManage.getGoodsInfoByNumber(c, h, !0), g ? g : ($.dialog({
								...
								data: {
									...
									callback: function(a, b, c) {
										"" === b && ($("#grid").jqGrid("addRowData", a, {}, "last"), A.newId = a + 1), setTimeout(function() {
				...
				afterSaveCell: function(a, b, c, d, e) {
					...
					case "goods":
						break;
					case "mainUnit":
						var f = $(".unitAuto").getCombo().getValue(),
							g = $("#" + a).data("goodsInfo");
						if (g.prices) for (var h = 0; h < g.prices.length; h++) if (g.prices[h].unitId == f) {
							var i = parseFloat($("#grid").jqGrid("getCell", a, e + 7)),
								j = g.prices[h].purPrice;
							if ($.isNumeric(j)) var k = $("#grid").jqGrid("setRowData", a, {
								unitId: f,
								price: j,
								amount: i * j
							});
						...
						break;
					case "qty":
						var j = $("#grid").jqGrid("getCell", a, e + 1);
						if (!isNaN(parseFloat(j))) {
							var k = $("#grid").jqGrid("setRowData", a, {
								amount: parseFloat(c) * parseFloat(j)
							});
							k && THISPAGE.calTotal()
						}
						break;
					case "price":
						var i = $("#grid").jqGrid("getCell", a, e - 1);
						if (!isNaN(parseFloat(i))) {
							var k = $("#grid").jqGrid("setRowData", a, {
								amount: parseFloat(c) * parseFloat(i)
							});
							k && THISPAGE.calTotal()
						}
						break;
					case "amount":
						var i = $("#grid").jqGrid("getCell", a, e - 2);
						if (!isNaN(parseFloat(i))) {
							var j = parseFloat(c) / parseFloat(i);
						...
						break;
					case "batch":
						var l = $("#grid").jqGrid("getRowData", a),
							g = $("#" + a).data("goodsInfo") || {};
						if (g.safeDays) {
							var m = {};
							if ($.trim(l.prodDate) || (m.prodDate = B), $.trim(l.safeDays) || (m.safeDays = g.safeDays), !$.trim(l.validDate)) {
								var n = l.prodDate || m.prodDate,
									o = n.split("-");
								if (n = new Date(o[0], o[1] - 1, o[2]), "Invalid Date" === n.toString()) return defaultPage.Public.tips({
									type: 2,
									content: "日期格式错误！"
								}), void setTimeout(function() {
									$("#grid").jqGrid("editCellByColName", a, "prodDate")
								}, 10);
								n && (n.addDays(Number(l.safeDays || m.safeDays)), m.validDate = n.format())
							}
							$.isEmptyObject(m) || $("#grid").jqGrid("setRowData", a, m)
						}
						break;
					case "prodDate":
						var l = $("#grid").jqGrid("getRowData", a),
							g = $("#" + a).data("goodsInfo") || {},
							m = {};
						$.trim(l.safeDays) || (m.safeDays = g.safeDays), $.trim(c) || (m.prodDate = B);
						var n = c || m.prodDate,
							o = n.split("-");
						if (n = new Date(o[0], o[1] - 1, o[2]), "Invalid Date" === n.toString()) return defaultPage.Public.tips({
							type: 2,
						...
						n && (n.addDays(Number(l.safeDays || m.safeDays)), m.validDate = n.format()), $("#grid").jqGrid("setRowData", a, m)
					}
				},
				loadonce: !0,
				resizeStop: function(a, b) {
					A.mod_PageConfig.setGridWidthByIndex(a, b, "grid")
				},
				footerrow: !0,
				userData: {
					goods: "合计：",
		...
		addEvent: function() {
			...
			this.customerCombo.input.enterKey(), this.$_date.bind("keydown", function(a) {
				...
				c && ("edit" === originalData.stata && (c.id = originalData.id, c.stata = "edit"), Public.ajaxPost("../scm/invOi.do?action=add&type=in", {
				...
				if (b.preventDefault(), Business.verifyRight("IO_UPDATE")) {
					var c = THISPAGE.getPostData();
					c && Public.ajaxPost("../scm/invOi.do?action=updateOi&type=in", {
						postData: JSON.stringify(c)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("IO_CHECK")) {
					var c = THISPAGE.getPostData();
					c && Public.ajaxPost("../scm/invOi.do?action=checkInvOi", {
						postData: JSON.stringify(c)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("IO_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invOi.do?action=rsBatchCheckInvOi", {
						id: originalData.id
					}, function(b) {
				...
				b.preventDefault();
				var c = THISPAGE.getPostData();
				c && Public.ajaxPost("../scm/invOi.do?action=addNew&type=in", {
					postData: JSON.stringify(c)
				}, function(b) {
					if (200 === b.status) {
				...
				return Business.verifyRight("IO_PRINT") ? void Public.print({
					title: "其他入库单打印",
					$grid: $("#grid"),
					pdf: "../scm/invOi.do?action=toOiPdf",
					billType: 10305,
					filterConditions: {
						id: originalData.id
					}
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invOi.do?action=updateIn&type=in", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invOi.do?action=updateIn&type=in", {
					id: a.salesListIds[a.idPostion]
				}, function(a) {
		...
		_getEntriesData: function(a) {
			...
			for (var b = [], c = $("#grid").jqGrid("getDataIDs"), d = 0, e = c.length; e > d; d++) {
				...
				if ("" !== h.goods) {
					...
					var l = ($("#" + g).data("unitInfo") || {}, $("#grid").jqGrid("getRowData", g));
					if (SYSTEM.ISSERNUM) {
						var m = i.serNumList;
...
$(function() {
	if (urlParam.id) hasLoaded || Public.ajaxGet("../scm/invOi.do?action=updateIn&type=in", {
		...
		if (originalData = {
			...
			$.extend(!0, originalData, a.data.warehouseData), originalData.callback = a.callback
		}
...
//			var c = "",
//				d = "";
//			billRequiredCheck && (c = '<a class="ui-btn" id="audit">审核</a>', d = '<a class="ui-btn" id="reAudit">反审核</a>'), this.btn_audit = c, this.btn_reaudit = d, this.btn_add = '<a id="savaAndAdd" class="ui-btn ui-btn-sp mrb">保存并新增</a><a id="save" class="ui-btn">保存</a>', this.btn_edit = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/invOi.do?action=toOiPdf&billType=IO" target="_blank" id="print" class="ui-btn mrb">打印</a><a id="edit" class="ui-btn">保存</a>', this.btn_view = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/invOi.do?action=toOiPdf&billType=IO" target="_blank" id="print" class="ui-btn mrb">打印</a>', this.btn_p_n = '<a class="ui-btn-prev mrb" id="prev" title="上一张"><b></b></a><a class="ui-btn-next" id="next" title="下一张"><b></b></a>', a.id > 0 ? (this.$_customer.data("contactInfo", {
//				id: a.buId,
//				name: a.contactName
//			}), this.customerCombo.input.val(a.contactName), this.$_number.text(a.billNo), this.$_date.val(a.date), a.description && this.$_note.val(a.description), $("#grid").jqGrid("footerData", "set", {
//				qty: a.totalQty,
