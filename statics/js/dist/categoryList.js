function initDom() {
	...
	var b, c = $(".ui-tab").on("click", "li", function(b) {
		var c = $(this),
			d = c.data("id"),
			e = c.html(),
			f = conditions.typeNumber,
			g = conditions.name;
		return conditions.typeNumber = d, conditions.name = e, verifyRight(rightsAction.query) ? ($(".cur").removeClass("cur"), c.addClass("cur"), $("#custom-assisting").getCombo().selectByIndex(0, !1), void a(d, e)) : (conditions.typeNumber = f, void(conditions.name = g))
	}),
		d = [],
		e = {
			customertype: "接收单位",
...
function postData(a) {
	...
	f.typeNumber = conditions.typeNumber, Public.ajaxPost("../basedata/assist.do?action=" + d, f, function(a) {
			...
			showParentCategory && (e = 150), this.dialog = $.dialog({
				...
				init: function() {
					var c = $(".hideFeild"),
						d = $("#ParentCategory"),
						e = $("#category");
					if (showParentCategory && (d.closest("li").show(), $("#ParentCategory").click(function(a) {
