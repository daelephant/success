		...
		addEvent: function() {
			...
			if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				if (a.preventDefault(), Business.verifyRight("TF_DELETE")) {
					var b = $(this).parent().data("id");
					$.dialog.confirm("您确定要删除该调拨单吗？", function() {
						Public.ajaxPost("../scm/invTf.do?action=delete", {
							id: b
						}, function(a) {
							if (200 === a.status && a.msg && a.msg.length) {
								var b = "<p>操作成功！</p>";
								for (var c in a.msg)"function" != typeof a.msg[c] && (c = a.msg[c], b += '<p class="' + (1 == c.isSuccess ? "" : "red") + '">调拨单［' + c.id + "］删除" + (1 == c.isSuccess ? "成功！" : "失败：" + c.msg) + "</p>");
								parent.Public.tips({
									content: b
								})
				...
				if (!Business.verifyRight("TF_DELETE")) return void a.preventDefault();
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join();
				return c ? void $.dialog.confirm("您确定要删除选中的调拨单吗？", function() {
					Public.ajaxPost("../scm/invTf.do?action=delete", {
						id: c
					}, function(a) {
						if (200 === a.status && a.msg && a.msg.length) {
				...
				queryConditions.matchCon = "请输入单据号或接收单位名或备注" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), queryConditions.outLocationId = -1, queryConditions.inLocationId = -1, THISPAGE.reloadData(queryConditions)
				...
				var f = "../scm/invTf.do?action=exportInvTf" + d;
				$(this).attr("href", f)
			}), billRequiredCheck) {
				...
				$(".wrapper").on("click", "#audit", function(a) {
					if (a.preventDefault(), Business.verifyRight("TF_CHECK")) {
						var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
							c = b.join();
						return c ? void Public.ajaxPost("../scm/invTf.do?action=batchCheckInvTf", {
							id: c
						}, function(a) {
					...
					if (Business.verifyRight("TF_UNCHECK")) {
						...
						var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
							c = b.join();
						return c ? void Public.ajaxPost("../scm/invTf.do?action=rsBatchCheckInvTf", {
							id: c
						}, function(a) {
							200 === a.status ? parent.Public.tips({
								content: a.msg
