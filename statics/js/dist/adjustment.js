var curRow, curCol, curArrears, loading, urlParam = Public.urlParam(),
	...
	THISPAGE = {
		init: function(a) {
			this.mod_PageConfig = Public.mod_PageConfig.init("adjustment"), SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_COSTAMOUNT || (hiddenAmount = !0), this.loadGrid(a), this.initDom(a), this.initCombo(), this.addEvent(), $.cookie("BarCodeInsert") && THISPAGE.$_barCodeInsert.addClass("active"), this.goodsEdittypeInit()
		},
		initDom: function(a) {
			...
			}), "edit" === a.status ? this.$_toolBottom.html('<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/invOi.do?action=toCBTZPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn mrb">打印</a><a id="edit" class="ui-btn">保存</a>') : this.$_toolBottom.html('<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/invOi.do?action=toCBTZPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn mrb">打印</a><a class="ui-btn-prev mrb" id="prev" title="上一张"><b></b></a><a class="ui-btn-next" id="next" title="下一张"><b></b></a>'), this.salesListIds = parent.salesListIds || [], this.idPostion = $.inArray(String(a.id), this.salesListIds), this.idLength = this.salesListIds.length, 0 === this.idPostion && $("#prev").addClass("ui-btn-prev-dis"), this.idPostion === this.idLength - 1 && $("#next").addClass("ui-btn-next-dis"), this.$_userName.html(a.userName), this.$_modifyTime.html(a.modifyTime), this.$_createTime.html(a.createTime)) : (this.$_toolBottom.html('<a id="savaAndAdd" class="ui-btn ui-btn-sp mrb">保存并新增</a><a id="save" class="ui-btn">保存</a>'), this.$_userName.html(SYSTEM.realName || ""), this.$_modifyTime.parent().hide(), this.$_createTime.parent().hide())
		},
		loadGrid: function(a) {
			function b(a, b, c) {
			...
			j.mod_PageConfig.gridReg(m, n), n = j.mod_PageConfig.conf.grids[m].colModel, $("#grid").jqGrid({
				...
				afterEditCell: function(a, b, c, d, e) {
					if ("goods" === b) {
						...
						if (f) {
							...
							f = $.extend(!0, {}, f), f.mainUnit = g.mainUnit, f.unitId = f.unitId, f.qty = g.qty, f.price = g.price, f.discountRate = g.discountRate, f.deduction = g.deduction, f.amount = g.amount, f.taxRate = g.taxRate, f.tax = g.tax, f.taxAmount = g.taxAmount, f.locationName = g.locationName, $("#" + a).data("goodsInfo", f)
						}
				...
				resizeStop: function(a, b) {
					j.mod_PageConfig.setGridWidthByIndex(a, b, "grid")
				},
				footerrow: !0,
				userData: {
					goods: "合计：",
		...
		reloadData: function(a) {
			function b() {
				c.customerCombo.selectByValue(a.buId, !1), c.$_date.val(a.date), c.$_number.text(a.billNo), a.note && c.$_note.val(a.note), c.$_userName.html(a.userName), c.$_modifyTime.html(a.modifyTime), c.$_createTime.html(a.createTime)
			}
			$("#grid").clearGridData();
			var c = this,
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				var c = $(this).siblings();
				setTimeout(function() {
					c.hasClass("unitAuto") ? c.trigger("click") : (a.storageCombo.active = !0, a.storageCombo.doQuery())
				}, 10)
			}), Business.billsEvent(a), $(".wrapper").on("click", "#save", function(b) {
				b.preventDefault();
				var c = THISPAGE.getPostData();
				c && ("edit" === originalData.stata && (c.id = originalData.id, c.stata = "edit"), Public.ajaxPost("../scm/invOi.do?action=addCADJ&type=cbtz", {
					postData: JSON.stringify(c)
				}, function(b) {
					200 === b.status ? (a.$_modifyTime.html((new Date).format("yyyy-MM-dd hh:mm:ss")).parent().show(), a.$_createTime.html((new Date).format("yyyy-MM-dd hh:mm:ss")).parent().show(), originalData.id = b.data.id, a.$_toolBottom.html('<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a id="edit" class="ui-btn mrb">保存</a><a href="../scm/invOi.do?action=toCBTZPdf&id=' + originalData.id + '" target="_blank" id="print" class="ui-btn">打印</a>'), parent.Public.tips({
						content: "保存成功！"
					})) : parent.Public.tips({
						type: 1,
						content: b.msg
				...
				if (b.preventDefault(), Business.verifyRight("CADJ_UPDATE")) {
					var c = THISPAGE.getPostData();
					c && Public.ajaxPost("../scm/invOi.do?action=updateCADJ&type=cbtz", {
						postData: JSON.stringify(c)
					}, function(b) {
				...
				b.preventDefault();
				var c = THISPAGE.getPostData();
				c && Public.ajaxPost("../scm/invOi.do?action=addNewCADJ&type=cbtz", {
					postData: JSON.stringify(c)
				}, function(b) {
					if (200 === b.status) {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invOi.do?action=updateCbtz&type=cbtz", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invOi.do?action=updateCbtz&type=cbtz", {
					id: a.salesListIds[a.idPostion]
				}, function(a) {
	...
	originalData;
$(function() {
	urlParam.id ? hasLoaded || Public.ajaxGet("../scm/invOi.do?action=updateCbtz&type=cbtz", {
		id: urlParam.id
	}, function(a) {
