	...
	originalData, THISPAGE = {
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				c && ("edit" === originalData.stata && (c.id = originalData.id, c.stata = "edit"), Public.ajaxPost("../scm/fundTf.do?action=add", {
				...
				if (b.preventDefault(), Business.verifyRight("ZJZZ_UPDATE")) {
					var c = THISPAGE.getPostData();
					c && Public.ajaxPost("../scm/fundTf.do?action=updateFundTf", {
						postData: JSON.stringify(c)
					}, function(b) {
				...
				b.preventDefault();
				var c = THISPAGE.getPostData();
				c && Public.ajaxPost("../scm/fundTf.do?action=addNew", {
					postData: JSON.stringify(c)
				}, function(b) {
					if (200 === b.status) {
...
$(function() {
	urlParam.id ? hasLoaded || Public.ajaxGet("../scm/fundTf.do?action=update", {
		id: urlParam.id
	}, function(a) {
