var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		init: function(a) {
			this.mod_PageConfig = Public.mod_PageConfig.init("salesOrder"), isAdmin !== !1 || SYSTEM.rights.AMOUNT_OUTAMOUNT || (hiddenAmount = !0, $("#amountArea").hide()), this.initDom(a), this.loadGrid(a), this.initCombo(), a.id > 0 && a.checked ? this.disableEdit() : (this.editable = !0, $("#grid").jqGrid("setGridParam", {
				cellEdit: !0
		...
		initDom: function(a) {
			...
			var f = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				g = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a id="copy" class="ui-btn">复制</a><a href="../scm/invSo.do?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a><a id="edit" class="ui-btn">保存</a>',
				h = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a id="copy" class="ui-btn">复制</a><a href="../scm/invSo.do?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
				i = "",
				j = "",
				k = "150602" == originalData.transType ? "生成退货单" : "生成出库单",
			...
			if (this.btn_add = f, this.btn_edit = g, this.btn_audit = i, this.btn_view = h, this.btn_reaudit = j, this.btn_turn = l, this.btn_close = m, this.btn_open = n, a.id > 0) {
		...
		setSaleByContact: function(a) {
			var b = this;
			b.salesCombo && Public.ajaxGet("../scm/invSo.do?action=findNearSoEmp", {
				buid: a.id
			}, function(a) {
						...
						if (d) {
							...
							if (f || (f = {}, a.$_customer.data("priceList", f)), e && "" !== $.trim(a.$_customer.find("input").val())) {
								var g = function() {
										...
										f[d.id] = a, Public.ajaxPost("../basedata/inventory.do?action=listBySelected", {
											type: "so",
											ids: d.id,
											contactId: e.id
										}, function(c) {
		...
		addEvent: function() {
			...
			this.customerCombo.input.enterKey(), this.$_date.bind("keydown", function(b) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost("../scm/invSo.do?action=add", {
				...
				var c = $(this),
					d = THISPAGE.getPostData();
				d && c.ajaxPost("../scm/invSo.do?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
					if (200 === b.status) {
			...
			}), $(".wrapper").on("click", "#copy", function(b) {
				if (b.preventDefault(), Business.verifyRight("SO_ADD")) {
					var c = $(this);
					a.$_date = $("#date").val(system.endDate), a.$_deliveryDate = $("#deliveryDate").val(system.endDate);
					...
					c.ajaxPost("../basedata/systemProfile.do?action=generateDocNo", {
						...
						if (200 === b.status) {
							originalData.id = -1, a.$_number.text(b.data.billNo), a.newId = 9, $("#mark").removeClass(), a.$_checkName.html(""), a.enableEdit(), $("#groupBtn").html(a.btn_edit + a.btn_audit), a.accountCombo.selectByValue(0, !0), a.$_modifyTime.html((new Date).format("yyyy-MM-dd hh:mm:ss")).parent().hide(), a.$_createTime.html((new Date).format("yyyy-MM-dd hh:mm:ss")).parent().hide(), parent.Public.tips({
								content: "复制数据成功,并生成新的单据！"
							});
							var c = a.btn_add;
							billRequiredCheck && (c += a.btn_audit), a.$_toolBottom.html('<span id="groupBtn">' + c + "</span>")
						} else parent.Public.tips({
							type: 1,
							content: b.msg
						})
				...
				if (b.preventDefault(), Business.verifyRight("SO_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invSo.do?action=updateInvSo", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("SO_CHECK")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invSo.do?action=checkInvSo", {
						postData: JSON.stringify(d)
					}, function(b) {
						200 === b.status ? (originalData.id = b.data.id, $("#mark").addClass("has-audit"), a.$_checkName.html(SYSTEM.realName).parent().show(), a.disableEdit(), $("#groupBtn").html(a.btn_turn + a.btn_view + a.btn_reaudit + a.btn_close), "150602" == a.classBox.getValue() ? $("#turn").html("生成退货单") : $("#turn").html("生成出库单"), parent.Public.tips({
							content: "审核成功！"
						})) : parent.Public.tips({
							type: 1,
							content: b.msg
				...
				if (b.preventDefault(), Business.verifyRight("SO_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invSo.do?action=rsBatchCheckInvSo", {
						id: originalData.id
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("SO_CLOSE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invSo.do?action=batchClose", {
						id: originalData.id
					}, function(b) {
						200 === b.status && b.msg && b.msg.length ? 1 == b.msg[0].isSuccess ? ($("#mark").addClass("has-closed"), a.disableEdit(), $("#groupBtn").html(a.btn_view + a.btn_open), "150602" == a.classBox.getValue() ? $("#turn").html("生成退货单") : $("#turn").html("生成出库单"), parent.Public.tips({
							content: "关闭成功！"
						})) : parent.Public.tips({
							type: 1,
				...
				if (b.preventDefault(), Business.verifyRight("SO_UNCLOSE")) {
					var c = $(this);
					c.ajaxPost("../scm/invSo.do?action=rebatchClose", {
						id: originalData.id
					}, function(b) {
						200 === b.status && b.msg && b.msg.length ? 1 == b.msg[0].isSuccess ? ($("#mark").removeClass("has-closed"), a.enableEdit(), $("#groupBtn").html((billRequiredCheck ? a.btn_turn : "") + a.btn_edit + a.btn_audit + a.btn_close), "150602" == a.classBox.getValue() ? $("#turn").html("生成退货单") : $("#turn").html("生成出库单"), parent.Public.tips({
							content: "开启成功！"
						})) : parent.Public.tips({
							type: 1,
				...
				if (b.preventDefault(), $(this).hasClass("ui-btn-dis")) return void parent.Public.tips({
					type: 1,
					content: "该订单已全部入库，不能生成出库单！"
				});
				var c = "出库单",
					d = "sales-sales",
					e = a.classBox.getValue();
				if ("150602" == e) var c = "出库退货单",
					d = "sales-salesBack";
				parent.tab.addTabItem({
					tabid: d,
					text: c,
					url: "../scm/invSa.do?action=initSale&id=" + originalData.id + "&flag=list&turn&transType=" + e
				}), parent.tab.reload(d)
			}), $("#prev").click(function(b) {
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSo.do?action=update", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSo.do?action=update", {
					id: a.idList[a.idPostion]
				}, function(a) {
...
$(function() {
	if (urlParam.id) {
		if (!hasLoaded) {
			var a = $(".bills").hide();
			Public.ajaxGet("../scm/invSo.do?action=update", {
				id: urlParam.id
			}, function(b) {
