function init() {
	SYSTEM.ISSERNUM || SYSTEM.enableAssistingProp || SYSTEM.ISWARRANTY || $("#tab").find(".tabItem:eq(2)").hide(), $("#number").focus(), void 0 !== cRowId ? Public.ajaxPost("../basedata/inventory.do?action=query", {
		id: cRowId
	}, function(a) {
		200 === a.status ? (rowData = a.data, "copy" == oper ? (initField(), initSkuGrid(), initGrid(), initCombo(), initEvent()) : (initField(), initSkuGrid(rowData.invSkus), initGrid(rowData.propertys), initCombo(), initEvent())) : parent.parent.Public.tips({
			type: 1,
			content: a.msg
		})
...
function postData() {
	var a = "";
	a = "add" == oper ? "新增商品" : "copy" == oper ? "复制商品" : "修改商品";
	...
	if (b) {
		...
		var d = "";
		d = "add" == oper || "copy" == oper ? "../basedata/inventory.do?action=add" : "../basedata/inventory.do?action=update", Public.ajaxPost(d, b, function(d) {
			if (200 == d.status) {
...
function initEvent() {
	...
	categoryTree = Public.categoryTree($category, b), $("#specs").blur(function() {
		...
		"" == a || "edit" == oper && a == rowData.spec || Public.ajaxPost("../basedata/inventory.do?action=checkSpec", {
			spec: a
		}, function(b) {
		...
		if (Business.verifyRight("FZSX_ADD")) {
			var b = function() {
					...
					a && Public.ajaxPost("../basedata/assistType.do?action=add", {
						name: a
					}, function(a) {
		...
		$("#gridWarning").data("newId", c), THISPAGE.newId_warning = c + 1;
		var d = {
		...
		$("#gridSku").data("newId", c), THISPAGE.newId_sku = c;
		var d = {
		...
		if (c && c.skuId && "edit" == oper) $.dialog.confirm("删除的分录将不能恢复，请确认是否删除？", function() {
			Public.ajaxPost("../basedata/inventory.do?action=deleteSku", {
				invId: rowData.id,
				skuId: c.skuId
			}, function(a) {
				if (200 === a.status) {
	...
	parent.$.dialog({
		...
		data: {
			...
			callback: function(a, b, c) {
				Public.ajaxPost("../basedata/invlocation.do?action=list", {}, function(b) {
					if (b && 200 == b.status) {
						var c = b.data.rows;
...
function initGrid(a) {
	...
	$grid.jqGrid({
		...
		afterEditCell: function(a, b, c, d, e) {
			...
			case "locationName":
				$("#" + d + "_locationName", "#grid").val(c), gridStoCombo.selectByText(c);
				break;
			case "skuName":
								...
								callback: function(b, c) {
									...
									if ($("#" + a).siblings(".jqgrow").each(function(a) {
										...
										if (b.length) {
											$("#" + a).data("serNumInfo", b);
											var e, f = b.length,
												g = parseFloat(Public.currencyToNum(i.unitCost));
...
function initWarningGrid() {
	if (THISPAGE.warningGrid) THISPAGE.warningGrid.closest(".grid-wrap").show();
		...
		var g = 720;
		$gridWarning.data("newId", a.length + 1), THISPAGE.warningGrid = $gridWarning.jqGrid({
			data: a,
			datatype: "clientSide",
			width: g,
			height: 264,
...
function initSkuGrid(a) {
	...
	if (THISPAGE.skuGrid) THISPAGE.skuGrid.closest(".grid-wrap").show();
		...
		$("#gridSku").data("newId", w.length + 1), THISPAGE.skuGrid = $gridSku.jqGrid({
			data: w,
			datatype: "clientSide",
			width: v,
			height: 160,
			...
			checkCode: function(a, b) {
				...
				return $.ajax({
					...
					success: function(a, b) {
						return a ? void(c = -1 == a.status ? rowData && rowData.barCode === c && "copy" !== oper ? !0 : "商品条码已经存在！" : !0) : !1
					},
					error: function(a) {
						c = "远程数据校验失败！"
					}
	...
	THISPAGE = {
		...
		handle: {
			getskuPropComboData: function() {
				...
				return "../basedata/assist.do?action=list&isDelete=2&typeNumber=" + a
			},
			formatter: {
