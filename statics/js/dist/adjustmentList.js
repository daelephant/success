var queryConditions = {
	...
	THISPAGE = {
		init: function(a) {
			SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_COSTAMOUNT || (hiddenAmount = !0), this.mod_PageConfig = Public.mod_PageConfig.init("adjustmentList"), this.initDom(), this.loadGrid(), this.addEvent()
		},
		initDom: function() {
		...
		loadGrid: function() {
			...
			this.mod_PageConfig.gridReg("grid", c), c = this.mod_PageConfig.conf.grids.grid.colModel, $("#grid").jqGrid({
				url: "../scm/invOi.do?action=listCbtz&type=cbtz",
		...
		addEvent: function() {
			...
			$(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				if (a.preventDefault(), Business.verifyRight("CADJ_DELETE")) {
					var b = $(this).parent().data("id");
					$.dialog.confirm("您确定要删除该成本调整单吗？", function() {
				...
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join();
				return c ? void $.dialog.confirm("您确定要删除选中的成本调整单吗？", function() {
					Public.ajaxPost("../scm/invOi.do?action=deleteCbtz", {
						id: c
					}, function(a) {
						if (200 === a.status && a.msg && a.msg.length) {
				...
				queryConditions.matchCon = "请输入单据号或接收单位名或备注" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), queryConditions.locationId = a.storageCombo.getValue(), THISPAGE.reloadData(queryConditions)
