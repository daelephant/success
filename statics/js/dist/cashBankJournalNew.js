define(["jquery", "print"], function(a, b, c) {
	function d() {
		Business.getSearchList(), Business.filterSettlementAccount(), Business.moreFilterEvent(), k("#date,#account").show(), k("#conditions-trigger").trigger("click"), k("#filter-fromDate").val(m.beginDate || ""), k("#filter-toDate").val(m.endDate || ""), m.beginDate && m.endDate && k("div.grid-subtitle").text("日期: " + m.beginDate + " 至 " + m.endDate), k("#filter-fromDate, #filter-toDate").datepicker(), Public.dateCheck(), k("#filter-submit").on("click", function(a) {
			a.preventDefault();
	...
	var k = a("jquery"),
		...
		n = "../report/bankBalance.do?action=exporter",
		o = "../report/bankBalance.do?action=detail";
	a("print");
