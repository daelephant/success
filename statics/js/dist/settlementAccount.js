$(function() {
	function a() {
		c.jqGrid({
			...
			url: "../basedata/settAcct.do?action=list",
			datatype: "json",
			...
			loadComplete: function(a) {
				if (a && 200 == a.status) {
					...
					$("#grid").data("gridData", b), 0 == a.items.length && parent.Public.tips({
						type: 2,
						content: "没有账户数据！"
					}), Public.ajaxPost("../basedata/settAcct.do?action=findAmountOver", {
						ids: c
					}, function(a) {
		...
		d = {
			operate: function(a, b) {
				...
				case "edit":
					...
					var d = this;
					Public.ajaxPost("../basedata/settAcct.do?action=query", {
						id: b
					}, function(b) {
				...
				case "del":
					...
					$.dialog.confirm("删除的账户将不能恢复，请确认是否删除？", function() {
						Public.ajaxPost("../basedata/settAcct.do?action=delete", {
							id: b
						}, function(a) {
