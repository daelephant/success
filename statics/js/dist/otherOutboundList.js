var queryConditions = {
	...
	THISPAGE = {
		init: function(a) {
			SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_COSTAMOUNT || (hiddenAmount = !0), this.mod_PageConfig = Public.mod_PageConfig.init("otherOutboundList"), this.initDom(), this.loadGrid(), this.addEvent()
		},
		initDom: function() {
		...
		loadGrid: function() {
			...
			this.mod_PageConfig.gridReg("grid", c), c = this.mod_PageConfig.conf.grids.grid.colModel, $("#grid").jqGrid({
				url: "../scm/invOi?action=listOut&type=out",
		...
		addEvent: function() {
			...
			if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				parent.tab.addTabItem({
					...
					text: "其他出库",
					url: "../storage/other_outbound?id=" + b
				});
				...
				if (a.preventDefault(), Business.verifyRight("OO_DELETE")) {
					var b = $(this).parent().data("id");
					$.dialog.confirm("您确定要删除该其他出库单吗？", function() {
						Public.ajaxPost("../scm/invOi?action=deleteOut", {
							id: b
						}, function(a) {
							if (200 === a.status && a.msg && a.msg.length) {
								var b = "<p>操作成功！</p>";
								for (var c in a.msg)"function" != typeof a.msg[c] && (c = a.msg[c], b += '<p class="' + (1 == c.isSuccess ? "" : "red") + '">其他出库单［' + c.id + "］删除" + (1 == c.isSuccess ? "成功！" : "失败：" + c.msg) + "</p>");
								parent.Public.tips({
									content: b
								})
				...
				if (!Business.verifyRight("OO_DELETE")) return void a.preventDefault();
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join();
				return c ? void $.dialog.confirm("您确定要删除选中的其他出库单吗？", function() {
					Public.ajaxPost("../scm/invOi?action=deleteOut", {
						id: c
					}, function(a) {
						if (200 === a.status && a.msg && a.msg.length) {
				...
				$(".wrapper").on("click", "#audit", function(a) {
					if (a.preventDefault(), Business.verifyRight("OO_CHECK")) {
						var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
							c = b.join();
						return c ? void Public.ajaxPost("../scm/invOi?action=batchCheckInvOo", {
							id: c
						}, function(a) {
					...
					if (a.preventDefault(), Business.verifyRight("OO_UNCHECK")) {
						var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
							c = b.join();
						return c ? void Public.ajaxPost("../scm/invOi?action=rsBatchCheckInvOo", {
							id: c
						}, function(a) {
							200 === a.status ? parent.Public.tips({
								content: a.msg
			...
			$("#search").click(function() {
				queryConditions.matchCon = "请输入单据号或接收单位名或备注" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), queryConditions.locationId = -1, queryConditions.transTypeId = -1, THISPAGE.reloadData(queryConditions)
				...
				if (!Business.verifyRight("OO_PRINT")) return void a.preventDefault();
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join(),
					d = c ? "&id=" + c : "",
					e = "../scm/invOi?action=toOoPdf" + d;
				$(this).attr("href", e)
				...
				var f = "../scm/invOi?action=exportInvOo" + d;
				$(this).attr("href", f)
			})
		}
	};
