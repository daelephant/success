		...
		addEvent: function() {
			...
			$(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				if (a.preventDefault(), Business.verifyRight("QTZC_DELETE")) {
					var b = $(this).parent().data("id");
					$.dialog.confirm("您确定要删除该其他支出单吗？", function() {
						Public.ajaxPost("../scm/ori.do?action=deleteExp", {
							id: b
						}, function(a) {
							if (200 === a.status && a.msg && a.msg.length) {
								var b = "<p>操作成功！</p>";
								for (var c in a.msg)"function" != typeof a.msg[c] && (c = a.msg[c], b += '<p class="' + (1 == c.isSuccess ? "" : "red") + '">其他支出单［' + c.id + "］删除" + (1 == c.isSuccess ? "成功！" : "失败：" + c.msg) + "</p>");
								parent.Public.tips({
									content: b
								})
				...
				if (!Business.verifyRight("QTZC_DELETE")) return void a.preventDefault();
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join();
				return c ? void $.dialog.confirm("您确定要删除选中的其他支出单吗？", function() {
					Public.ajaxPost("../scm/ori.do?action=deleteExp", {
						id: c
					}, function(a) {
						if (200 === a.status && a.msg && a.msg.length) {
				...
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join(),
					d = c ? "&id=" + c : "",
					e = "../scm/ori.do?action=toPdf&transType=153402" + d;
				$(this).attr("href", e)
				...
				var f = "../scm/ori.do?action=exportExp" + d;
				$(this).attr("href", f)
