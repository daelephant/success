var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		init: function(a) {
			"150602" == urlParam.transType ? this.mod_PageConfig = Public.mod_PageConfig.init("salesBack") : this.mod_PageConfig = Public.mod_PageConfig.init("sales"), isAdmin !== !1 || SYSTEM.rights.AMOUNT_OUTAMOUNT || (hiddenAmount = !0, $("#amountArea").hide()), this.initDom(a), this.loadGrid(a), this.initCombo(), a.id > 0 && a.checked ? this.disableEdit() : (this.editable = !0, $("#grid").jqGrid("setGridParam", {
				cellEdit: !0
		...
		initDom: function(a) {
			...
			if (this.$_customer = $("#customer"), this.$_date = $("#date").val(system.endDate), this.$_number = $("#number"), this.$_note = $("#note"), this.$_discountRate = $("#discountRate"), this.$_deduction = $("#deduction"), this.$_discount = $("#discount"), this.$_payment = $("#payment"), this.$_arrears = $("#arrears"), this.$_totalArrears = $("#totalArrears"), this.$_toolTop = $("#toolTop"), this.$_toolBottom = $("#toolBottom"), this.$_paymentTxt = $("#paymentTxt"), this.$_accountInfo = $("#accountInfo"), this.$_userName = $("#userName"), this.$_modifyTime = $("#modifyTime"), this.$_createTime = $("#createTime"), this.$_checkName = $("#checkName"), this.$_customerFree = $("#customerFree"), this.customerArrears = 0, this.$_note.placeholder(), "150602" == originalData.transType) {
				parent.$("#page-tab").find("li.l-selected").children("a").html("出库退货单");
				$("#paymentTxt").html("本次退款")
			} else {
				parent.$("#page-tab").find("li.l-selected").children("a").html("出库单");
			...
			}), a.description && this.$_note.val(a.description), this.$_discountRate.val(a.disRate), this.$_deduction.val(a.disAmount), this.$_discount.val(a.amount), this.$_payment.val(a.rpAmount), this.$_arrears.val(a.arrears), this.$_customerFree.val(a.customerFree), requiredMoney && ($("#accountWrap").show(), isAdmin !== !1 || SYSTEM.rights.SettAcct_QUERY ? this.accountCombo = Business.accountCombo($("#account"), {
				width: 112,
				height: 300,
				emptyOptions: !0,
				addOptions: {
			...
			var f = '<!--<a id="original" class="ui-btn ui-btn-sc">选择源单</a>--><a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				g = '<!--<a id="original" class="ui-btn ui-btn-sc">选择源单</a>--><a id="add" class="ui-btn ui-btn-sp">新增</a><a id="copy" class="ui-btn">复制</a><a href="../scm/invSa.do?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a><a id="edit" class="ui-btn">保存</a>',
				h = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a id="copy" class="ui-btn">复制</a><a href="../scm/invSa.do?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
				i = "",
				j = "",
				k = "";
			...
			if (f += k, this.btn_add = f, this.btn_edit = g, this.btn_audit = i, this.btn_view = h, this.btn_reaudit = j, this.btn_tempSave = k, a.id > 0) {
		...
		loadGrid: function(a) {
			...
			var B = [{
				...
				nameExt: '<small id="batchStorage"></small>',
				width: 100,
				editable: !0,
				edittype: "custom",
				editoptions: {
		...
		setSaleByContact: function(a) {
			var b = this;
			b.salesCombo && Public.ajaxGet("../scm/invSa.do?action=findNearSaEmp", {
				buid: a.id
			}, function(a) {
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				var c = $(this),
					d = THISPAGE.getPostData();
				d && c.ajaxPost("../scm/invSa.do?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
					if (200 === b.status) {
				...
				if (b.preventDefault(), Business.verifyRight("SA_ADD")) {
					var c = $(this);
					...
					c.ajaxPost("../basedata/systemProfile.do?action=generateDocNo", {
						...
						if (200 === b.status) {
							originalData.id = -1, a.$_number.text(b.data.billNo), a.newId = 9, $("#mark").removeClass(), a.$_checkName.html(""), a.enableEdit(), $("#groupBtn").html(a.btn_edit + a.btn_audit), a.accountCombo.selectByValue(0, !0), a.$_modifyTime.html((new Date).format("yyyy-MM-dd hh:mm:ss")).parent().hide(), a.$_createTime.html((new Date).format("yyyy-MM-dd hh:mm:ss")).parent().hide(), $("[aria-describedby=grid_srcOrderNo]").html(""), parent.Public.tips({
								content: "复制数据成功！"
							}), isCopy = !0;
							var c = a.btn_add;
				...
				if (b.preventDefault(), Business.verifyRight("SA_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invSa.do?action=updateInvSa", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("SA_CHECK")) {
					...
					d && c.ajaxPost("../scm/invSa.do?action=checkInvSa", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("SA_UNCHECK")) {
					var c = $(this);
					c.ajaxPost("../scm/invSa.do?action=rsBatchCheckInvSa", {
						id: originalData.id
					}, function(b) {
				...
				if (a.preventDefault(), Business.verifyRight("SA_ADD")) {
					var b = "出库单",
						c = "sales-sales";
					if ("150602" == originalData.transType) var b = "出库退货单",
						c = "sales-salesBack";
					parent.tab.overrideSelectedTabItem({
						tabid: c,
						text: b,
						url: "../scm/invSa.do?action=initSale&transType=" + originalData.transType
					})
				}
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSa.do?action=update", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSa.do?action=update", {
					id: a.idList[a.idPostion]
				}, function(a) {
				...
				if ("" !== h.goods) {
					...
					if (i) {
						...
						f = {
							...
							serNumList: m
						}, isCopy && (f.srcOrderEntryId = "", f.srcOrderId = "", f.srcOrderNo = ""), SYSTEM.ISWARRANTY && $.extend(!0, f, {
							batch: h.batch || "",
							prodDate: h.prodDate || "",
							safeDays: h.safeDays || "",
