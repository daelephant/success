...
var THISPAGE = {
	...
	initDom: function(a) {
		...
		var b = '<a id="savaAndAdd" class="ui-btn ui-btn-sp mrb">保存并新增</a><a id="save" class="ui-btn">保存</a>',
			c = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/receipt.do?action=toPdf" target="_blank" id="print" class="ui-btn mrb">打印</a><a id="edit" class="ui-btn mrb">保存</a>',
			d = '<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/receipt.do?action=toPdf" target="_blank" id="print" class="ui-btn mrb">打印</a><b></b></a>',
			e = "",
			f = "",
			g = "";
	...
	initCombo: function() {
		this.accountCombo = $(".accountAuto").combo({
			data: "../basedata/settAcct.do?action=list",
			ajaxOptions: {
		...
		var a = originalData.id > 0 ? "../scm/invSa.do?action=findUnhxList&buId=" + originalData.buId + "&id=" + originalData.id : [];
	...
	addEvent: function() {
		...
		this.customerCombo.input.enterKey(), this.$_note.enterKey(), this.$_date.bind("keydown", function(a) {
			...
			if (Business.verifyRight("RECEIPT_ADD")) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/receipt.do?action=add", {
			...
			if (b.preventDefault(), Business.verifyRight("RECEIPT_UPDATE")) {
				...
				c && Public.ajaxPost("../scm/receipt.do?action=updateReceipt", {
			...
			if (Business.verifyRight("RECEIPT_ADD")) {
				var d = THISPAGE.getPostData();
				d && (c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/receipt.do?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
			...
			return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
				type: 2,
				content: "已经没有上一张了！"
			}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), a.idPostion && (loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/receipt.do?action=update", {
			...
			return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
				type: 2,
				content: "已经没有下一张了！"
			}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), a.idPostion && (loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/receipt.do?action=update", {
				id: a.paymentListIds[a.idPostion]
			}, function(a) {
			...
			if (b.preventDefault(), Business.verifyRight("RECEIPT_CHECK")) {
				...
				d && c.ajaxPost("../scm/receipt.do?action=checkReceipt", {
					postData: JSON.stringify(d)
				}, function(b) {
			...
			if (b.preventDefault(), Business.verifyRight("RECEIPT_UNCHECK")) {
				var c = $(this);
				c.ajaxPost("../scm/receipt.do?action=rsbatchCheckReceipt", {
					id: originalData.id
				}, function(b) {
	...
	originalData;
$(function() {
	urlParam.id ? hasLoaded || Public.ajaxGet("../scm/receipt.do?action=update", {
		id: urlParam.id
	}, function(a) {
