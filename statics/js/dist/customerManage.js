function init() {
	void 0 !== cRowId ? Public.ajaxPost("../basedata/contact.do?action=query", {
		id: cRowId
	}, function(a) {
		200 == a.status ? (rowData = a.data, initField(), initEvent(), initGrid(rowData.links)) : parent.$.dialog({
			title: "系统提示",
			content: "获取接收单位数据失败，暂不能修改接收单位，请稍候重试",
			icon: "alert.gif",
			max: !1,
			min: !1,
			cache: !1,
	...
	$_form.validator({
		...
		valid: function(a) {
			...
			Public.ajaxPost("../basedata/contact.do?action=checkName", {
				...
				-1 == a.status ? parent.$.dialog.confirm('接收单位名称 "' + b + '" 已经存在！是否继续？', function() {
					postCustomerData()
...
function postCustomerData() {
	var a = "add" == oper ? "新增接收单位" : "修改接收单位",
		b = getCustomerData(),
		c = b.firstLink || {};
	delete b.firstLink, Public.ajaxPost("../basedata/contact.do?action=" + ("add" == oper ? "add" : "update"), b, function(d) {
		if (200 == d.status) {
	...
	Business.verifyRight("BUTYPE_ADD") && parent.$.dialog({
		...
		data: {
			...
			callback: function(a, b, c) {
				categoryCombo.loadData("../basedata/assist.do?action=list&typeNumber=customertype", ["id", a.id]), c && c.api.close()
			}
		},
		width: 400,
		height: 100,
