		...
		addEvent: function() {
			...
			$(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				parent.tab.addTabItem({
					...
					text: "资金转账单",
					url: "../money/accountTransfer?id=" + b
				});
				...
				if (a.preventDefault(), Business.verifyRight("ZJZZ_DELETE")) {
					var b = $(this).parent().data("id");
					$.dialog.confirm("您确定要删除该资金转账单吗？", function() {
						Public.ajaxPost("../scm/fundTf.do?action=delete", {
							id: b
						}, function(a) {
							if (200 === a.status && a.msg && a.msg.length) {
								var b = "<p>操作成功！</p>";
								for (var c in a.msg)"function" != typeof a.msg[c] && (c = a.msg[c], b += '<p class="' + (1 == c.isSuccess ? "" : "red") + '">资金转账单［' + c.id + "］删除" + (1 == c.isSuccess ? "成功！" : "失败：" + c.msg) + "</p>");
								parent.Public.tips({
									content: b
								})
				...
				if (!Business.verifyRight("ZJZZ_DELETE")) return void a.preventDefault();
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join();
				return c ? void $.dialog.confirm("您确定要删除选中的资金转账单吗？", function() {
					Public.ajaxPost("../scm/fundTf.do?action=delete", {
						id: c
					}, function(a) {
						if (200 === a.status && a.msg && a.msg.length) {
				...
				a.preventDefault(), Business.verifyRight("ZJZZ_ADD") && parent.tab.addTabItem({
					tabid: "money-accountTransfer",
					text: "资金转账单",
					url: "../scm/fundTf.do?action=initFundTf"
				})
				...
				var f = "../scm/fundTf.do?action=export" + d;
				$(this).attr("href", f)
