function initFilter() {
	...
	filterConditions = {
		...
		customer: a.customer || "",
		supplier: a.supplier || ""
	}, filterConditions.matchCon ? $("#matchCon").val(filterConditions.matchCon || "请输入接收单位、供应商或编号查询") : ($("#matchCon").addClass("ui-input-ph"), $("#matchCon").placeholder()), filterConditions.customer && $("#customer").attr("checked", !0), filterConditions.supplier && $("#supplier").attr("checked", !0), $("#search").on("click", function(a) {
		a.preventDefault();
		var b = "请输入接收单位、供应商或编号查询" === $("#matchCon").val() ? "" : $.trim($("#matchCon").val());
		filterConditions = {
			matchCon: b,
...
function initField() {
	var a = filterConditions.customer ? filterConditions.customer.split(",") : "",
		...
		c = "";
	a && b ? c = "「您已选择了<b>" + a.length + "</b>个接收单位，<b>" + b.length + "</b>个商品进行查询」" : a ? c = "「您已选择了<b>" + a.length + "</b>个接收单位进行查询」" : b && (c = "「您已选择了<b>" + b.length + "</b>个商品进行查询」"), $("#cur-search-tip").html(c)
}
function initEvent() {
