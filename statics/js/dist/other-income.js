var curRow, curCol, curArrears, loading, urlParam = Public.urlParam(),
	...
	originalData, THISPAGE = {
		...
		initDom: function(a) {
			this.$_customer = $("#customer"), this.$_date = $("#date").val(SYSTEM.endDate), this.$_number = $("#number"), this.$_note = $("#note"), this.$_toolTop = $("#toolTop"), this.$_toolBottom = $("#toolBottom"), this.$_amount = $("#amount").val(a.amount || 0), this.$_accountInfo = $("#accountInfo"), this.$_userName = $("#userName"), this.$_modifyTime = $("#modifyTime"), this.$_createTime = $("#createTime"), this.customerCombo = Business.billCustomerCombo($("#customer"), {
				defaultSelected: 0,
				emptyOptions: !0
		...
		addEvent: function() {
			...
			this.customerCombo.input.enterKey(), this.$_date.bind("keydown", function(a) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/ori.do?action=addInc", {
					postData: JSON.stringify(d)
				}, function(b) {
					c.removeClass("ui-btn-dis"), 200 === b.status ? (a.$_modifyTime.html((new Date).format("yyyy-MM-dd hh:mm:ss")).parent().show(), a.$_createTime.html((new Date).format("yyyy-MM-dd hh:mm:ss")).parent().show(), originalData.id = b.data.id, urlParam.id = b.data.id, a.$_toolBottom.html('<a id="add" class="ui-btn ui-btn-sp mrb">新增</a><a href="../scm/ori.do?action=toPdf&transType=153401" target="_blank" id="print" class="ui-btn mrb">打印</a><a id="edit" class="ui-btn mrb">保存</a>'), parent.Public.tips({
						content: "保存成功！"
					})) : parent.Public.tips({
						type: 1,
						content: b.msg
				...
				if (b.preventDefault(), Business.verifyRight("QTSR_UPDATE")) {
					var c = THISPAGE.getPostData();
					c && Public.ajaxPost("../scm/ori.do?action=updateInc", {
				...
				var d = THISPAGE.getPostData();
				d && (c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/ori.do?action=addNewInc", {
					postData: JSON.stringify(d)
				}, function(b) {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/ori.do?action=updateInc", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/ori.do?action=updateInc", {
					id: a.salesListIds[a.idPostion]
				}, function(a) {
...
$(function() {
	urlParam.id ? hasLoaded || Public.ajaxGet("../scm/ori.do?action=getIncDetail", {
		id: urlParam.id
	}, function(a) {
