...
class Inventory extends CI_Controller {
	...
	public function index() {
		switch ($this->action) {
			case 'list':
			    $this->inventoryList();break;  
			case 'listCache':
			    $this->listCache();break;  
			case 'add':
			    $this->add();break;	
			case 'update':
			    $this->update();break;
			...
			case 'exporter':
			    $this->exporter();break; 
			case 'deleteSku':
			    $this->deleteSku();break;
			case 'checkBarCode':
			    $this->checkBarCode();break; 
			case 'listBySelected':
			    $this->listBySelected();break;  
			case 'warningExporter':
			    $this->warningExporter();break;  
			case 'listInventoryQtyWarning':
			    $this->listInventoryQtyWarning();break;	
	...
	private function inventoryList() {
		...
		$where = '(a.isDelete=0)';
		$where .= strlen($ids)>0 ? ' and id in('.$ids.')' :'';
	...
	//商品选择 最近出售价格
	private function listBySelected() { 
	...
	//获取图片信息
	private function getImagesById() {
	    $id = intval($this->input->post('id',TRUE));
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['status']       = 1;
			$v[$arr]['name']         = $row['name'];
			$v[$arr]['url']          = site_url().'/basedata/inventory?action=getImage&pid='.$row['id'];
			$v[$arr]['thumbnailUrl'] = site_url().'/basedata/inventory?action=getImage&pid='.$row['id'];
			$v[$arr]['deleteUrl']    = '';
			$v[$arr]['deleteType']   = '';
		}
		$json['status'] = 200;
	...
	private function uploadImages() {
		...
		$files[0]['status']       = 1;
		$files[0]['size']         = (float)$info['size'];
		$files[0]['name']         = $info['name'];
		$files[0]['url']          = site_url().'/basedata/inventory?action=getImage&pid='.$newid;
		$files[0]['thumbnailUrl'] = site_url().'/basedata/inventory?action=getImage&pid='.$newid;
		$files[0]['deleteUrl']    = '';
		$files[0]['deleteType']   = '';
		$json['status'] = 200;
		$json['msg']    = 'success';
	...
	//保存上传图片信息
	private function addImagesToInv() {
	...
	private function update(){
		...
		if ($data) {
			...
			if (strlen($data['propertys'])>0) {                          
				...
				foreach ($list as $arr=>$row) {
					$v[$arr]['invId']         = $data['id'];
					$v[$arr]['locationId']    = isset($row['locationId']) ? intval($row['locationId']) : 0;
					$v[$arr]['qty']           = isset($row['quantity']) ? (float)$row['quantity']:0; 
					$v[$arr]['price']         = isset($row['unitCost']) ? (float)$row['unitCost']:0; 
					$v[$arr]['amount']        = isset($row['amount']) ? (float)$row['amount']:0; 
					$v[$arr]['skuId']         = isset($row['skuId']) ? intval($row['skuId']):0;  
					$v[$arr]['billDate']      = '2000-01-01';
					$v[$arr]['billNo']        = '期初数量';
					$v[$arr]['billType']      = 'INI';
					$v[$arr]['transTypeName'] = '期初数量';
	...
	private function delete(){
		...
		if (count($data) > 0) {
		    ...
		    if ($sql) {
				...
				str_alert(200,'success',array('msg'=>'','id'=>explode(',',$id)));
			}
			str_alert(-1,'删除失败');
		}
	}
	...
	private function disable(){
		...
		if ($disable==1) {
		    ...
		    $this->common_model->checkpurview(96);
		}
	...
	//库存预警 
	private function listinventoryqtywarning() {
		...
		if ($assistId > 0) {
		    $cid = array_column($this->mysql_model->get_results('category','(isDelete=0) and find_in_set('.$assistId.',path)'),'id'); 
			if (count($cid)>0) {
			    $cid = join(',',$cid);
	...
	private function warningExporter() {
	    $this->common_model->checkpurview();
	...
	private function get_goods_info($id) {
		...
		if (count($data)>0) {
			...
			$data['josl']          = ''; 
			$data['highQty']       = intval($data['highQty']); 
			$data['lowQty']        = intval($data['lowQty']);
