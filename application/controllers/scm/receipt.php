...
class Receipt extends CI_Controller {
	...
	public function index() {
		switch ($this->action) {
			case 'initReceipt':
			    $this->load->view('scm/receipt/initReceipt');break;  
			case 'initUnhxList':
			    $this->load->view('scm/receipt/initUnhxList');break; 
			case 'initReceiptList':
			    $this->load->view('scm/receipt/initReceiptList');break;
			case 'list':
			    $this->receiptList();break;	
			case 'add':
			    $this->add();break;	
			case 'addNew':
			    $this->addNew();break;
			...
			case 'delete':
			    $this->delete();break;	
			case 'updateReceipt':
			    $this->updateReceipt();break;
			case 'batchCheckReceipt':
			    $this->batchCheckReceipt();break;	
			case 'rsbatchCheckReceipt':
			    $this->rsbatchCheckReceipt();break;	
			case 'toPdf':
			    $this->toPdf();break;	
			case 'checkReceipt':
			    $this->checkReceipt();break;		
			case 'exportReceipt':
			    $this->exportReceipt();break;				
			default:  
			    str_alert(-1,'非法请求');
		}
	}
	
	private function receiptList() {
	...
	private function exportReceipt(){
	...
	private function updateReceipt(){
		...
		if (strlen($data)>0) {
			$data = $this->validform((array)json_decode($data, true));
			$info = elements(array(
						'billType','transType','transTypeName','buId','billDate',
    ...
    private function delete() {
	    ...
	    $id   = $this->input->get_post('id',TRUE) ? str_enhtml($this->input->get_post('id',TRUE)) : 0;
		$data = $this->mysql_model->get_results('invoice','(id in('.$id.')) and transType=153001 and isDelete=0');  
		if (count($data)>0) {
