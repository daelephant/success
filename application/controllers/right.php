...
class Right extends CI_Controller {
	...
	public function index() {
		switch ($this->action) {
			case 'isMaxShareUser':
			    $this->isMaxShareUser();break;	
			case 'queryAllUser':
			    $this->queryAllUser();break;
			case 'queryUserByName':
			    $this->queryUserByName();break;
			case 'addUser':
			    $this->addUser();break;
			case 'queryAllUserRight':
			    $this->queryAllUserRight();break;
			case 'auth2UserCancel':
			    $this->auth2UserCancel();break;
			case 'auth2User':
			    $this->auth2User();break;	
			case 'addRights2OutUser':
			    $this->addRights2OutUser();break;		
			default:  
			    str_alert(-1,'非法请求');
		}
	}
	...
	//判断用户名是否存在
	private function queryUserByName() {
	...
	//更新权限
	private function addRights2OutUser() {
	    $userName = str_enhtml($this->input->get_post('userName',TRUE));
	...
	//详细权限设置
	private function queryAllUserRight() {
	    $userName = str_enhtml($this->input->get_post('userName',TRUE));
		$data = $this->mysql_model->get_rows('admin',array('username'=>$userName));
		if (count($data)>0) {
	...
	//停用
	private function auth2UserCancel(){
	    $userName = str_enhtml($this->input->get_post('userName',TRUE));
		$data = $this->mysql_model->get_rows('admin',array('username'=>$userName));
		if (count($data)>0) {
