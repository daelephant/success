...
<link href="<?php echo base_url()?>statics/css/<?php echo sys_skin()?>/default.css?ver=20151110" rel="stylesheet" type="text/css" id="defaultFile">
<script src="<?php echo skin_url()?>/js/common/plugins/ligerui/base.js?ver=20151110"></script>
<script src="<?php echo skin_url()?>/js/common/plugins/ligerui/jquery.menu.js?ver=20151110"></script>
<script src="<?php echo skin_url()?>/js/common/plugins/ligerui/jquery.tab.js?ver=20151110"></script>
<!-- author：357058607 -->
...
function syncOnlineStoreInfo(){
	if('false' === 'true'){
		...
		var sid = "";
		var nickName = "";
		var url = WDURL+'/online-store/syncOnlineStoreInfo?language=zh-CHS&site=SITE_MAIN&siId='+SYSTEM.DBID+'&scheme='+SCHEME+'&logonName='+SYSTEM.userName;
		var _dlg = $.dialog({
			cancle: false,
			lock: true,
			width: 400,
...
function getGoods() {
	if(SYSTEM.isAdmin || SYSTEM.rights.INVENTORY_QUERY) {
		...
		Public.ajaxGet('<?php echo site_url('basedata/inventory')?>?action=listCache&isDelete=2', { rows: 5000 }, function(data){
			if(data.status === 200) {
...
function getBatch(){
	...
	if(SYSTEM.isAdmin || SYSTEM.ISWARRANTY) {
		Public.ajaxGet('<?php echo site_url('warranty')?>?action=getBatchNoList', {}, function(data){
			if(data.status === 200) {
...
function markupVension(){
	var imgSrcList = {
				base:'<?php echo base_url()?>statics/css/<?php echo sys_skin()?>/img/icon_v_b.png',	//
				baseExp:'<?php echo base_url()?>statics/css/<?php echo sys_skin()?>/img/icon_v_b_e.png',	//
				baseTrial:'<?php echo base_url()?>statics/css/<?php echo sys_skin()?>/img/icon_v_b_t.png',	//
				standard:'<?php echo base_url()?>statics/css/<?php echo sys_skin()?>/img/icon_v_s.png', //
				standardExp:'<?php echo base_url()?>statics/css/<?php echo sys_skin()?>/img/icon_v_s_e.png', //
				standardTrial :'<?php echo base_url()?>statics/css/<?php echo sys_skin()?>/img/icon_v_s_t.png' //
			};
	...
	if(SYSTEM.siType === 1){
		switch(SYSTEM.siVersion){
			...
			case 2:	imgModel.attr('src',imgSrcList.baseExp).attr('alt','');
				break;
			case 3: imgModel.attr('src',imgSrcList.base).attr('alt','');//
				break;
			case 4: imgModel.attr('src',imgSrcList.baseExp).attr('alt','');//
				break;
		};
	} else {
		switch(SYSTEM.siVersion){
			case 1:	imgModel.attr('src',imgSrcList.standardTrial).attr('alt','');
				break;
			case 3: imgModel.attr('src',imgSrcList.standard).attr('alt','');//
				break;
			case 4: imgModel.attr('src',imgSrcList.standardExp).attr('alt','');//
				break;
		};
	};
...
<div id="container" class="cf">
  <div id="col-side">
    ...
    <li class="item item-vip"> <a href="javascript:void(0);" class="vip main-nav">高级<span class="arrow">&gt;</span></a>
      ...
      <li class="item item-sales"> <a href="javascript:void(0);" class="sales main-nav">出库<span class="arrow">&gt;</span></a>
        <div class="sub-nav-wrap single-nav">
...
<script src="<?php echo base_url()?>statics/js/dist/default.js?ver=20151110"></script>
