...
.container{margin:0 auto; width:800px;padding-top: 50px;}
			    ...
			    <div class="l">
				    <ul class="ul-inline">
				        ...
				        <li><a class="ui-btn mrb" id="search">查询</a>
				        	<a href="#" class="ui-btn mrb" id="export">导出</a>
				        </li>
				        <li>
					    	<div class="ui-btn-menu js-hideDetailList">
			                    <a class="ui-btn menu-btn mrb" style="width: 85px;" href="#" id="toPo">生成采购订单<b></b></a>
			                    <div class="con more-operate-con" style="padding: 5px 13px;line-height: 30px;font-size: 14px;width:95px;">
			                    	<ul class="more-operate cf" style="border-style:dashed; border-width:1px; border-color:#ccc;">
				                        <li><a href="#" id="toPur" data-type="2" class="js-batchEdit">生成采购单</a></li>
				                    </ul>
			                    </div>
			                </div>
			            </li>
...
$(function(){
	...
	var queryConditions = {},SYSTEM = system = parent.parent.SYSTEM, ticketsType = {};
	var THISPAGE = {
		init:function(){
			this.initDom();
			this.initEvent();
		...
		initEvent:function(){
			...
			$('#export').on('click',function(e){
				...
				var url = '../basedata/inventory.do?action=warningExporter&isDelete=2'+params;
				$(this).attr('href', url);
			});
			//单据切换
			$('.js-hideDetailList').hover(function() {
			// $('.more-operate-con').css("display","block");
				$('.more-operate-con').addClass("ui-btn-menu-cur");
			}, function() {
				$('.more-operate-con').removeClass('ui-btn-menu-cur').addClass('ui-btn-menu');
			});
			//生成采购单
			$('#toPur').on('click',function(e){
				...
				if(rowList.length){
					...
					parent.parent.tab.addTabItem({tabid: tabid, text: title, url: '../scm/invPu.do?action=initPur&id=-1&goodsIds=' + JSON.stringify(rowList) + '&flag=list&turnBygoodList&transType='+ transType});
			...
			$('#toPo').on('click',function(e){
				...
				if(rowList.length){
					var title = '采购订单';
					var tabid = 'purchase-purchaseOrder';
					var transType = '150501';
					parent.parent.tab.addTabItem({tabid: tabid, text: title, url: '../scm/invPo.do?action=initPo&id=-1&goodsIds=' + JSON.stringify(rowList) + '&flag=list&turnBygoodList=1&transType='+ transType});//billType=PO
					parent.parent.tab.reload(tabid);
				}else{
					parent.parent.Public.tips({type:2, content:'请先选择商品！'});
				}
		...
		initGrid:function(){
			$("#grid").jqGrid({
				url:'../basedata/inventory.do?action=listInventoryQtyWarning',
				datatype: "json",
				width: 600,
				height: 240,
				gridview: true,
			...
			$("#grid_batch").jqGrid({
				url:'../warranty.do?action=getAdvancedList',
				datatype: "json",
				width: 758,
				height: 280,
				gridview: true,
