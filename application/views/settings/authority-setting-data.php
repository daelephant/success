...
$(function(){
	...
	Public.ajaxPost('../dataright?action=dt&userName='+userName, {}, function(data){
			if(data.status === 200) {
	...
	thisPage = {
		...
		eventHandle : function(){
			$start.click(function(){
				...
				Public.ajaxPost('../dataright?action=ar&userName=' + userName ,{rightid:'{"rightids":['+ rightids.join(',') + ']}'}, function(data){
					if(data.status === 200) {	
						if($start.hasClass('ui-label-default')){
							$search.trigger('click');
							$start.removeClass('ui-label-default').html('已启用').attr('title','点击关闭该类别权限');
			...
			$('#save').click(function(e){
				  ...
				  Public.ajaxPost('../dataright?action=update&type='+ type +'&userName=' + userName , {rights:'['+ strRights.join() + ']'}, function(data){
					  if(data.status === 200) {
			...
			$search.on('click', function(e){
				...
				$grid.jqGrid('setGridParam',{page: 1, url:'../dataright?action=query', datatype: "json", postData: queryConditions}).trigger("reloadGrid");
			});
